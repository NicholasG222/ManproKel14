...
class EditPengumuman : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        db.collection("tbPengumuman").document(intentGrup!!.original!!).get().addOnSuccessListener { result ->
    ...
    private fun uploadImage() {
        ...
        if (filePath != null) {
            ...
            val ann =
                Pengumuman(gambar, editNama.text.toString(),"Edited on ${date}", editIsi.text.toString(),original)

            ann.judul?.let { it1 ->
                db.collection("tbPengumuman").document(original).set(ann)
    ...
    private fun addUploadRecordToDb(uri: String) {
        val ann =
            Pengumuman(uri, editNama.text.toString(),"Edited on ${date}", editIsi.text.toString(),original)
