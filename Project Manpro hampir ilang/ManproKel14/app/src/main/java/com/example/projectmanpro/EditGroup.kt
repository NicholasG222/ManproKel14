...
class EditGroup : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        db.collection("tbGrup").document(intentGrup!!.original!!).get().addOnSuccessListener { result ->
    ...
    private fun uploadImage() {
        ...
        if (filePath != null) {
            ...
            val group = Grup(gambar, editNama.text.toString(),kategori, currEmail,original)

            group.nama?.let { it1 ->
                db.collection("tbGrup").document(original).set(group)
    ...
    private fun addUploadRecordToDb(uri: String) {
        val group =
            Grup(uri, editNama.text.toString(),kategori, currEmail,original)
